...
function LoadKendoHandlers() {
    ...
    $.fn.kExcludeAndHide = function () {
        this.hide();
        this.removeAttr('required');
    }

    $.fn.kIncludeAndShowInBlock = function (addStyle = '') {
        this.attr('style', 'display:inline-block;' + addStyle);
        this.prop('required', true);
        this.children().each(function () {
            var $childElement = $(this);
            $childElement.prop('required', true);
        }
        )
    }

    $.fn.input = function (newValue) {
        var oldValue = $(this).val();
        if (arguments.length == 0)
            return oldValue;
        var result = $(this).val(newValue);
